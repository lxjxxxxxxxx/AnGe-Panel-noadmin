...
func LoginInterceptor(c *gin.Context) {
	...
	// 没有token信息视为未登录
	if cToken == "" {
		// 尝试 Bearer token (accessToken)
		if LoginByBearerToken(c) {
			return
		}
...
// LoginByBearerToken 通过 Authorization: Bearer <accessToken> 直接认证
func LoginByBearerToken(c *gin.Context) bool {
	...
	hash := cmn.Sha256(accessToken)
	record, err := (&models.UserAccessToken{}).GetByTokenHash(hash)
	if err != nil {
		return false
	}
